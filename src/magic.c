...
void spell_create_portal( int sn, int level, CHAR_DATA *ch, void *vo )
{
    CHAR_DATA	*victim;
    OBJ_DATA	*origin;
    OBJ_DATA	*destination;

    if ( !( victim = get_char_world( ch, target_name ) )
	|| ( IS_NPC( victim ) ) )
    {	/* victim not found, or victim is a mob */
	send_to_char( "No such location.\n\r", ch );
	return;
    }

    origin = create_object( get_obj_index( OBJ_VNUM_PORTAL ), get_trust( ch ) );
    origin->value[0] = victim->in_room->vnum;
    origin->value[1] = 0;
    origin->value[2] = 0;
    origin->timer = 1;
    obj_to_room( origin, ch->in_room );

    destination = create_object( get_obj_index( OBJ_VNUM_PORTAL ), get_trust( ch ) );
    destination->value[0] = ch->in_room->vnum;
    destination->value[1] = 0;
    destination->value[2] = 0;
    destination->timer = 1;
    obj_to_room( destination, victim->in_room );

    act( "$p suddenly appears.", ch, origin, NULL, TO_ROOM );
    act( "$p suddenly appears.", ch, origin, NULL, TO_CHAR );
    act( "$p suddenly appears.", victim, destination, NULL, TO_ROOM );
    act( "$p suddenly appears.", victim, destination, NULL, TO_CHAR );
...
void spell_acid_breath( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	for ( obj_lose = victim->carrying; obj_lose; obj_lose = obj_next )
	{
	    ...
	    case ITEM_STAFF:
	    case ITEM_WAND:
		act( "$p corrodes and breaks.",victim,obj_lose,NULL,TO_CHAR);
		extract_obj( obj_lose );
		break;
	    case ITEM_SCROLL:
		act ("$p is burned into waste.",victim,obj_lose,NULL,TO_CHAR);
		extract_obj( obj_lose );
		break;
	    }
	}
...
void spell_fire_breath( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	for ( obj_lose = victim->carrying; obj_lose; obj_lose = obj_next )
	{
	    ...
	    case ITEM_LIGHT:     msg = "$p overheats and explodes!"; break;
	    }
