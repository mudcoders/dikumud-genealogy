...
*/
{
	...
	if (!IS_AFFECTED(ch, AFF_SNEAK)
	   && IS_SET(world[ch->in_room].room_flags,ARENA) ) {
		switch(ch->specials.arena) {
			case ARENA_BLUE_PLR: case ARENA_BLUE_CAPT:
				act("[BLUE] $n has arrived.",TRUE,ch,0,0,TO_ROOM);
				break;
			case ARENA_RED_PLR: case ARENA_RED_CAPT:
				act("[RED] $n has arrived.",TRUE,ch,0,0,TO_ROOM);
				break;
			default:
	...
	if (IS_SET(world[ch->in_room].room_flags, DEATH) && !IS_TRUSTED(ch)){ 
		death_cry(ch);
		extract_char(ch);
		return(-1);
	}
...
void do_follow(struct char_data *ch, char *argument, int cmd)
{
	...
	if (IS_AFFECTED(ch, AFF_CHARM) && (ch->master)) {
		...
		if (leader == ch) {
			...
			diff=GET_LEVEL(ch)-GET_LEVEL(leader);
			if ( (diff>6)||(diff<-6) && !IS_TRUSTED(ch) ){
				send_to_char("You can't follow someone with more than 6 levels difference!\n\r",ch);
				return;
			}
			if (ch->master)
				stop_follower(ch);
